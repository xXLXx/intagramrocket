...
*/
	...
	else if(isset($_GET['gateshop']) && isset($_GET['amount']))
	{
		 ...
		 global $wpdb;
			...
			$code=$_COOKIE['codes'];
			$likeperpage=$_COOKIE['likeperpage'];
...
*/
   			 ...
   			 global $wpdb;
	         $sql = "SELECT * FROM createorder where ID=".$_REQUEST['item_number_1'];
						...
						foreach ($array as $key ) {
							...
							if($c<$count)
							{
								$image_url='https://www.instagram.com/p/'.$key.'/';
									...
									$params = array('service' => $result1->service_id, 'link' =>$image_url, 'quantity' => $likeperpage);
		...
		else if($process=='follower')
		{
			$url=get_site_url().'/follower?getdone=1';
		...
		{
			$url=get_site_url().'/service-likes?getdone=1';
...
*/
 ...
 <div id="laoding" style="display: none;"><img src="<?php echo plugins_url( '/image/loading.gif', __FILE__ );?>"></div>
		           		...
		           		<button onclick="load_more();">Load more</button></div>
			...
			var urls='https://www.instagram.com/'+username+'/media/';
			  ...
			  jQuery.ajax({
	            ...
	            success: function(jsonp) { 
	            	var jsonp = jQuery.parseJSON(jsonp);
