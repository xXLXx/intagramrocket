...
$fh       =fopen('orderUpdated.txt', 'w')  or die("Utyftyftf");
fwrite($fh, $data);
...
mail('kamini_thakur@esferasoft.com', 'Order updated on sales fundraising', json_decode($data));
...
foreach ($array->line_items as $key => $value) {
  ...
  if( (!empty($value->product_id)) && $financial_status == 'paid' && $fulfillment_status == 'fulfilled' ) {
        ...
        else
        {
            ...
            echo $updateMetafield=$sc->call('PUT','/admin/products/'.$value->product_id.'/metafields/'.$metafield_id.'.json',$meta);  
        }               
  } 
}
