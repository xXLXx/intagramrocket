...
function EWD_URP_Meta_Box( $post ) {
	...
	if ($InDepth_Reviews == "No" or sizeOf($Review_Categories_Array) == 0) {
		...
		foreach ($Review_Categories_Array as $Review_Categories_Item) {
			...
			elseif ($Review_Categories_Item['CategoryType'] == "Date") {echo "<input type='date' id='ewd-urp-" . $Review_Categories_Item['CategoryName'] . "' name='EWD_URP_" . $Review_Categories_Item['CategoryName'] . "' value='" . esc_attr( $Value ) . "' />";}
