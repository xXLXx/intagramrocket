...
function EWD_URP_Display_Review($Review, $Review_Params = array()) {
	...
	elseif ($Review_Format == "Image") {$Body_Class = 'ewd-urp-review-div ewd-urp-image-review';}
	...
	if ($Group_By_Product == "No" and isset($Review_Params['product_name']) and $Review_Params['product_name'] == "") {
		...
		$ReturnString .= __("Review for ", 'ultimate-reviews') . $Product_Name;
		$ReturnString .= "</div>";
	}
	...
	$ReturnString .= "<div class='ewd-urp-review-title' id='ewd-urp-title-" . $Unique_ID . "-" . $Review->ID . "' data-postid='" . $Unique_ID . "-" . $Review->ID . "' itemprop='name'>" . $Review->post_title . "</div>";
	...
	if ($Display_Author == "Yes"  or $Display_Date == "Yes" or $Display_Categories == "Yes") {
		...
		if ($Display_Date == "Yes") {$ReturnString .= $On_Label . " <span class='ewd-urp-date' itemprop='datePublished' content='" . $Review->post_date . "'>" . $Review->post_date . "</span> ";}
		if ($Display_Categories == "Yes") {$ReturnString .= "<div class='ewd-urp-categories'>Categories: " . implode(", ", wp_get_post_terms($Review->ID, 'urp-review-category', array('fields' => 'names'))) . "</div> ";}
		$ReturnString .= "</div>";
	}
	...
	if ($InDepth_Reviews == "Yes" and isset($Review_Categories_Array[0]['CategoryName']) and $Review_Categories_Array[0]['CategoryName'] != "") {
		...
		foreach ($Review_Categories_Array as $Review_Category_Item) {
			if ($Review_Category_Item['CategoryName'] != "" and $Review_Category_Item['CategoryType'] != "Default") {
				...
				if ($Review_Category_Item['CategoryType'] == "ReviewItem" or $Review_Category_Item['CategoryType'] == "") {$ReturnString .= $Review_Category_Item['CategoryName'] . " " . $Score_Label . ": ";}
